<template>
  <AdminLayout pageTitle="Ürünler">
    <div class="page-header">
      <h2>Ürün Listesi</h2>
      <button class="primary-btn" @click="$router.push('/products/add')">
      ...
      <table v-else class="data-table">
        ...
        <tbody>
          <tr v-for="product in products" :key="product.id">
            ...
            <td>
              <div class="actions">
                <button class="action-btn action-edit" @click="$router.push('/products/' + product.id + '/edit')">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <button class="action-btn action-toggle" @click="toggleProductStatus(product)">
                  {{ product.active ? 'Pasife Al' : 'Aktife Al' }}
                </button>
                <button class="action-btn action-delete" @click="deleteProduct(product.id)">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
const fetchProducts = async (page = currentPage.value) => {
  loading.value = true
  try {
    const response = await apiFetch(`/api/products?page=${page}&limit=100`)
